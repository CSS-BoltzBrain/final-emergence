...
class TestSimulationSmoke:
    ...
    def test_simulation_creation_empty(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.1)
        assert sim is not None

    def test_simulation_creation_surround(self):
        """Test Simulation creation with surround.yaml."""
        sim = Simulation('configs/surround.yaml', num_agents=1, adjust_probability=0.1)
        assert sim is not None
...
class TestSimulationInitialization:
    ...
    def test_num_agents_stored(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=5, adjust_probability=0.1)
...
class TestSimulationAgentSpawning:
    ...
    def test_spawn_agents_creates_list(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=3, adjust_probability=0.1)
...
class TestSimulationUpdate:
    ...
    def test_update_modifies_agent_map(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=2, adjust_probability=0.1)
        initial_sum = np.sum(sim._state_map.get_agent_map())
        sim.update()
        # Agent map should have some changes (could be same if no movement)
        assert isinstance(np.sum(sim._state_map.get_agent_map()), (int, np.integer))
...
class TestSimulationSaveLoad:
    ...
    def test_load_checkpoints(self, tmp_path):
        ...
        # Create and save
        sim1 = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.1)
        sim1.update()
        sim1.checkpoint()
        sim1.update()
        sim1.checkpoint()
        ...
        # Load
        sim2 = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.1)
    ...
    def test_save_load_roundtrip(self, tmp_path):
        ...
        sim1 = Simulation('configs/empty.yaml', num_agents=2, adjust_probability=0.1)
        for _ in range(3):
            sim1.update()
            sim1.checkpoint()
...
class TestSimulationIntegration:
    ...
    def test_simulation_agent_lifecycle(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.1)
        initial_agent_count = len([a for a in sim._agent_list if a is not None])
...
class TestSimulationEdgeCases:
    ...
    def test_simulation_zero_agents(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=0, adjust_probability=0.1)
        sim.update()
    ...
    def test_simulation_high_adjust_probability(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.9)
        sim.update()
        # Should work without error
        assert True
    ...
    def test_simulation_zero_adjust_probability(self):
        ...
        sim = Simulation('configs/empty.yaml', num_agents=1, adjust_probability=0.0)
        sim.update()
        # Should work without error
        assert True
